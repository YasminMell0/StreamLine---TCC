...
if(!empty($_GET['id'])){
    $id = $_GET['id'];
    $sql = mysqli_query($con,"SELECT Id_prof, `ativo` FROM `cadastroprofissional` WHERE Id_prof = $id");
    ...
    header("Location: telaAdm.php");
            ...
            if (isset($_POST['editar'])) { 
                ...
                $update = mysqli_query($con,"UPDATE cadastroprofissional SET ativo = '$valorativo' WHERE Id_prof = $id");

                if($update >= 1){
                  header('Location: telaAdm.php?autonomo=autonomo&filtro=filtro');
