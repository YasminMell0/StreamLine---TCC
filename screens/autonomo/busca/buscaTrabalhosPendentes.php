...
if(!isset($_GET['busca'])){
    header("Location: ../TrabalhosPendentes.php");
    exit;
}
...
WHERE C.ativo = 1 and S.data_finalizacao = 0 and S.Id_prof = $id_prof and C.nome_cli LIKE :nome LIMIT $pagina, $itens_pagina");
...
A.profissao, A.esp_exp, A.preco_fixo, S.data_contratacao FROM servico S 
INNER JOIN cadastroprofissional A ON S.Id_prof = A.Id_prof 
INNER JOIN cadastrocliente C ON S.Id_cli = C.Id_cli 
WHERE S.hora_finalizacao = 0 and S.Id_prof = $id_prof")->num_rows;
