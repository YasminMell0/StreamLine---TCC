...
if (!empty($dados['dataPublicacao'])) {
  ...
  and S.Id_cli = $id_cli ORDER BY S.data_contratacao DESC LIMIT $pagina, $itens_pagina;";
  $consulta = $con->query($cons);
} elseif (!empty($dados['avali'])) {
  $cons = "SELECT P.Id_prof, P.nome_prof, P.genero, P.cidade, P.estado, P.profissao, A.Avaliacao_prof,
  P.biografia, P.foto_perfil, S.data_contratacao, S.preco_final, S.data_contratacao 
  from servico S 
  INNER JOIN cadastroprofissional P ON S.Id_prof = P.Id_prof 
  INNER JOIN avaliacoesprofissional A ON S.Id_prof = A.Id_prof
  WHERE S.hora_finalizacao = 0 
  and S.Id_cli = $id_cli ORDER BY A.Avaliacao_prof DESC LIMIT $pagina, $itens_pagina;";
  $consulta = $con->query($cons);
  ...
  S.Id_prof = P.Id_prof
  WHERE S.hora_finalizacao = '00:00:00' 
  and S.Id_cli = $id_cli ORDER BY P.cidade and P.estado DESC LIMIT $pagina, $itens_pagina;";
  $consulta = $con->query($cons);
  ...
  S.Id_prof = P.Id_prof
  WHERE S.hora_finalizacao = '00:00:00' 
  and S.Id_cli = $id_cli ORDER BY S.preco_final DESC LIMIT $pagina, $itens_pagina;";
  $consulta = $con->query($cons);
  ...
  S.Id_prof = P.Id_prof
  WHERE S.hora_finalizacao = '00:00:00' 
  and S.Id_cli = $id_cli ORDER BY S.preco_final ASC LIMIT $pagina, $itens_pagina;";
  $consulta = $con->query($cons);
} else {
...
$num_total = $con->query("SELECT P.Id_prof, P.nome_prof, P.genero, P.cidade, P.estado, P.profissao, A.Avaliacao_prof,
A.Id_prof, P.biografia, P.foto_perfil, S.data_contratacao, S.preco_final, S.data_contratacao 
from servico S INNER JOIN cadastroprofissional P ON 
S.Id_prof = P.Id_prof 
INNER JOIN avaliacoesprofissional A ON S.Id_cli = A.Id_cli
WHERE S.hora_finalizacao = 0 
and S.Id_cli = $id_cli")->num_rows;
    ...
    <div class="row flex-nowrap">
      <div class="col-auto col-md-3 col-xl-2 px-sm-2 px-0 bg-faded" style="background-color: #152c40">
        <div class="d-flex flex-column align-items-center align-items-sm-start px-3 pt-2 min-vh-100">
          ...
          <ul class="nav nav-pills flex-column mb-sm-auto mb-0 align-items-center align-items-sm-start" id="menu">
            <form action="buscaCliente/buscaTrabalhosPendentes.php" method="GET" name="buscaAutonomo">
      ...
      <div class="col py-3">
        <div class="container">
          ...
          <!-- filtro -->
          <form action="TrabalhosPendentesCliente.php" method="get" name="filtrobusca">
                  ...
                  <form method="post" action="ClienteFinalizarServico.php" id="form" name="form">
                    <div class="row">
                      <div class="col-sm-4">
                        ...
                        <center>
                          <img src="<?php echo '../autonomo/perfil/' . $dado['foto_perfil'] ?>" class="img-fluid" alt="user" style="width: 150px; height: 150px; object-fit: cover; margin-top: 3%; margin-bottom: 5%; border-radius: 50%;">
                        </center>
                      </div>
                ...
                <div class="col-md-5 col-sm-6 grid-margin stretch-card">
                  <nav>
                    <ul class="pagination d-flex justify-content-center flex-wrap pagination-rounded-flat pagination-success">
                      <li class="page-item"><a class="page-link" href="TrabalhosPendentesCliente.php?pagina=0" data-abc="true"><i class="fa fa-angle-left"></i></a>
                      </li>
                      ...
                      ?>
                        <li <?php echo $estilo; ?> class="page-item active"><a class="page-link" href="TrabalhosPendentesCliente.php?pagina=<?php echo $i; ?>" data-abc="true">
                            <?php echo $i + 1; ?>
                          </a></li>
                      <?php } ?>
                      <!-- Fim da numeração -->
                      <li class="page-item"><a class="page-link" href="TrabalhosPendentesCliente.php?pagina=<?php echo $num_paginas - 1; ?>" data-abc="true"><i class="fa fa-angle-right"></i></a></li>
                    </ul>
                  </nav>
                </div>
              </div>
