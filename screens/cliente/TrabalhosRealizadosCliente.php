...
$itens_pagina = 5; // quantidade por página
...
if (!empty($dados['dataPublicacao'])) {
  $cons = "SELECT A.Id_prof, A.nome_prof, A.genero, A.estado, A.cidade, A.foto_perfil, A.biografia, A.profissao,
  A.preco_fixo, S.data_contratacao FROM servico S 
  INNER JOIN cadastroprofissional A ON S.Id_prof = A.Id_prof
  WHERE S.hora_finalizacao > 0 and S.Id_cli = $id_cli ORDER BY S.data_contratacao DESC LIMIT $pagina, $itens_pagina;";
  $consulta = $con->query($cons);
} elseif (!empty($dados['avali'])) {
  $cons = "SELECT A.Id_prof, A.nome_prof, A.genero, A.estado, A.cidade, A.foto_perfil, A.biografia, A.profissao,
  A.preco_fixo, E.Avaliacao_prof, S.data_contratacao FROM servico S 
  INNER JOIN cadastroprofissional A ON S.Id_prof = A.Id_prof
  INNER JOIN avaliacoesprofissional E ON S.Id_prof = E.Id_prof 
  WHERE S.hora_finalizacao > 0 and S.Id_cli = $id_cli ORDER BY E.Avaliacao_prof DESC LIMIT $pagina, $itens_pagina;";
  $consulta = $con->query($cons);
} elseif (!empty($dados['local'])) {
  $cons = "SELECT A.Id_prof, A.nome_prof, A.genero, A.estado, A.cidade, A.foto_perfil, A.biografia, A.profissao,
  A.preco_fixo,  S.data_contratacao FROM servico S 
  INNER JOIN cadastroprofissional A ON S.Id_prof = A.Id_prof 
  WHERE S.hora_finalizacao > 0 and S.Id_cli = $id_cli ORDER BY A.cidade and A.estado DESC LIMIT $pagina, $itens_pagina;";
  $consulta = $con->query($cons);
} elseif (!empty($dados['prealto'])) {
  $cons = "SELECT A.Id_prof, A.nome_prof, A.genero, A.estado, A.cidade, A.foto_perfil, A.biografia, A.profissao,
  A.preco_fixo, S.data_contratacao FROM servico S 
  INNER JOIN cadastroprofissional A ON S.Id_prof = A.Id_prof 
  WHERE S.hora_finalizacao > 0 and S.Id_cli = $id_cli ORDER BY S.preco_final DESC LIMIT $pagina, $itens_pagina;";
  $consulta = $con->query($cons);
} elseif (!empty($dados['prebaixo'])) {
  $cons = "SELECT A.Id_prof, A.nome_prof, A.genero, A.estado, A.cidade, A.foto_perfil, A.biografia, A.profissao,
  A.preco_fixo, S.data_contratacao FROM servico S 
  INNER JOIN cadastroprofissional A ON S.Id_prof = A.Id_prof 
  WHERE S.hora_finalizacao > 0 and S.Id_cli = $id_cli ORDER BY S.preco_final ASC LIMIT $pagina, $itens_pagina;";
  $consulta = $con->query($cons);
} else {
...
A.preco_fixo, S.data_contratacao FROM servico S 
INNER JOIN cadastroprofissional A ON S.Id_prof = A.Id_prof
WHERE S.hora_finalizacao > 0 and S.Id_cli = $id_cli LIMIT $pagina, $itens_pagina;";
  $consulta = $con->query($cons);
}
      ...
      <div class="col-auto col-md-3 col-xl-2 px-sm-2 px-0 bg-faded" style="background-color: #152c40">
        <div class="d-flex flex-column align-items-center align-items-sm-start px-3 pt-2 min-vh-100">
          ...
          <ul class="nav nav-pills flex-column mb-sm-auto mb-0 align-items-center align-items-sm-start" id="menu">
            <form action="buscaCliente/buscaTrabalhosRealizados.php" method="GET" name="buscaAutonomo">
          ...
          <div class="card">
            <div class="card-body">
              ...
              ?>
                ...
                <h4 style="text-align: center;">Nenhum Resultado foi encontrado!</h4>
              <?php   }
              ?>
            </div>
          </div>
                ...
                <div class="col-md-5 col-sm-6 grid-margin stretch-card">
                  <nav>
                    <ul class="pagination d-flex justify-content-center flex-wrap pagination-rounded-flat pagination-success">
                      <li class="page-item"><a class="page-link" href="TrabalhosRealizadosCliente.php?pagina=0" data-abc="true"><i class="fa fa-angle-left"></i></a>
                      </li>
                      ...
                      ?>
                        <li <?php echo $estilo; ?> class="page-item active"><a class="page-link" href="TrabalhosRealizadosCliente.php?pagina=<?php echo $i; ?>" data-abc="true">
                            <?php echo $i + 1; ?>
                          </a></li>
                      <?php } ?>
                      <!-- Fim da numeração -->
                      <li class="page-item"><a class="page-link" href="TrabalhosRealizadosCliente.php?pagina=<?php echo $num_paginas - 1; ?>" data-abc="true"><i class="fa fa-angle-right"></i></a></li>
                    </ul>
                  </nav>
                </div>
              </div>
