...
$sql = mysqli_query ($con, "INSERT INTO `denuncia_prof`(`Id_denuncia`, `Id_prof`, `Id_cli`, `motivo`, 
`reclamacao`, `data_denuncia`) VALUES ('','$id_prof','$id_cli','$motivo',
'$reclamacao','$data_denuncia')");
    ...
    WHERE Id_cli = $id_cli group by Id_cli HAVING COUNT(Id_cli) = 3"); // Consulta da tabela denúncia
        ...
        if($sql >= 1){
            if($quant == 1){
                $up = mysqli_query ($con, "UPDATE `cadastroprofissional` SET `ativo`='0' WHERE Id_prof = $id_prof");
...
<p style="margin: 0; font-size: 14px; text-align: center; mso-line-height-alt: 34px;"><span style="font-size:17px;">ESTE E-MAIL É AUTOMÁTICO, POR FAVOR NÃO RESPONDA!</span></p>
            ...
            header('Location: ../cliente/denunciarAutonomo.php');
        }
