...
$tempo_exp = $_POST['tempo'];
$espTempo = $_POST['espTempo'];
$esp_exp= implode(', ',(array)$_POST['esp']);
...
$contato = preg_replace( '/[^a-z0-9]/i', '', $contato);
...
switch ($profissao) {
 ...
 $confirmaremail = mysqli_query($con, "SELECT * FROM `cadastroprofissional` WHERE email_prof = '$email'");
 ...
 if($totalEmail == 1){
    echo "Este E-mail já foi cadastrado!\nPor favor, faça login.";
        ...
        $sql = mysqli_query($con, "INSERT INTO `cadastroprofissional`(`Id_prof`, `nome_prof`, `genero`, 
        `email_prof`, `senha`, `profissao`, `tempo_exp`, `espTempo`, `esp_exp`, `biografia`, 
        `data_nascimento`, `data_cadProf`, `contato`, `cep`, `numero_casa`, `complemento`, `cidade`, 
        `estado`, `link`, `preco_fixo`, `foto_perfil`, `ativo`, `token`) VALUES ('','$nome','$genero',
        '$email','$senha','$prof','$tempo_exp','$espTempo','$esp_exp','Olá!','$data_nasc',
        '$data_cad','$contato','$cep','$num_casa','$complemento','$cidade','$estado',
        '$link','$preco','perfis1.svg', '0', '$token')");
...
<p style="margin: 0; font-size: 14px; text-align: center; mso-line-height-alt: 21px;">Copyright© 2022 LMNNY</p>
            ...
            if (mail($for, $assunto, $corpo, implode("\r\n", $headers))) {
                ...
                header("Location: ../email/confirmarCadAuto.php");
            } else {
            }
        }else{
